...
function render(originalSong) {
    ...
    if(state.meta.interlude) { pinInt.style.display = 'block'; pinInt.innerHTML = `<span style="opacity:0.7">INTER:</span> ` + renderSimple(state.meta.interlude, sh); } else { pinInt.style.display = 'none'; }
    var scIntro = document.getElementById('scrollIntro');
    if(state.meta.intro) { scIntro.style.display = 'block'; scIntro.innerHTML = `<span style="opacity:0.7">INTRO:</span> ` + renderSimple(state.meta.intro, sh); } else { scIntro.style.display = 'none'; }

    // Main Body
    var outDiv = document.getElementById('outputContent');
    outDiv.innerHTML = ""; 
    state.parsedChords.forEach(L => {
        if(L.type === 'br') { var d = document.createElement('div'); d.style.height = "10px"; outDiv.appendChild(d); return; }
        if(L.type === 'lyricOnly') { var p = document.createElement('div'); p.className = 'compact-line'; p.innerText = L.text; outDiv.appendChild(p); return; }
    ...
    // Εμφάνιση κουμπιού Save Tone μόνο αν έχουμε transpose
    var btnSaveTone = document.getElementById('btnSaveTone');
    btnSaveTone.style.display = (state.t !== 0) ? 'block' : 'none';

    // QR Generate
    var qrDiv = document.getElementById('playerQR');
    qrDiv.innerHTML = ""; 
    if(originalSong && typeof QRCode !== 'undefined') {
        try { setTimeout(() => { new QRCode(qrDiv, { text: JSON.stringify(originalSong), width: 128, height: 128, correctLevel: QRCode.CorrectLevel.L }); }, 50); } catch(e) {}
    }
}
